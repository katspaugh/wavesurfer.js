...
/**
 * Apply fade-in to channel data (returns new array)
 */
...
/**
 * Apply fade-out to channel data (returns new array)
 */
export function applyFadeOut(
  ...
  const result = new Float32Array(channelData.length)
  const fadeStart = Math.max(0, channelData.length - fadeSamples)
