...
WaveSurfer.util = {
    ...
    parsePlaylist: function (plfile,pltype) {
        // check if playlist type is given
        if (pltype == 'm3u' || 'audio/mpegurl') {
            var playlist = plfile.replace(/^.*#.*$|#EXTM3U|#EXTINF:/mg, '').split('\n');
        } else if (pltype == 'pls' || 'audio/x-scpls') {
            // to do
        } else if (pltype == 'smil' || pltype == 'application/smil'){
            // to do
        } else if (pltype == 'json' || pltype == 'application/json'){
		    // to do
	    } else {
            return [];
        }
